...
// LocalStorage キー基底 (令和1年度用) - ユーザープレフィックスは UserManager が付与
const STORAGE_BASE_KEYS = {
    wrongAnswers: 'wrong_r1',
    stats: 'stats_r1',
...
<head>
    <meta charset="UTF-8">
    <title>1級建築施工管理技術検定 令和元年度 問題集</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Hiragino Kaku Gothic ProN', 'Yu Gothic', sans-serif; font-size: 11pt; line-height: 1.6; padding: 20mm; }
        ...
        .question-header { font-weight: bold; margin-bottom: 8px; padding: 5px 10px; background: #f0f0f0; border-left: 4px solid #2563eb; }
        .question-text { margin-bottom: 10px; padding-left: 10px; }
        .choices { padding-left: 20px; }
        .choice { margin-bottom: 5px; display: flex; }
        .choice-num { min-width: 25px; font-weight: bold; }
        .answer-section { margin-top: 40px; page-break-before: always; }
        .answer-section h2 { font-size: 14pt; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 2px solid #333; }
        .answer-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px; }
        .answer-item { padding: 5px 10px; background: #f9f9f9; border: 1px solid #ddd; text-align: center; font-size: 10pt; }
        @media print {
            body { padding: 15mm; }
...
<body>
    <div class="header">
        <h1>1級建築施工管理技術検定 学科試験</h1>
        <p>令和元年度 問題集（全${questions.length}問）</p>
    </div>
`;
    ...
    questions.forEach((q, index) => {
        html += `
    <div class="question">
        <div class="question-header">問題 ${q.id}</div>
        <div class="question-text">${escapeHtml(q.question)}</div>
        <div class="choices">
`;
        q.choices.forEach((choice, i) => {
            html += `            <div class="choice"><span class="choice-num">${i + 1}.</span><span>${escapeHtml(choice)}</span></div>\n`;
        });
        html += `        </div>
    </div>
`;
...
`;
    questions.forEach(q => {
        html += `            <div class="answer-item">問${q.id}: ${q.correct}</div>\n`;
    });
    html += `        </div>
    </div>
</body>
