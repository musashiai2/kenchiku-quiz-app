...
const STORAGE_BASE_KEYS = {
    ...
    history: 'history_r3',
    adaptiveLearning: 'adaptive_r3',
    studyTime: 'quiz_study_time_r3'
};
...
function displayReviewHint(question, wrongData) {
    ...
    if (wrongCount >= 3) { hintText = 'この問題は複数回間違えています。'; tipsList.push('解説をよく読んで、なぜ間違えたか考えましょう'); tipsList.push('選択肢の違いを明確に理解することが重要です'); }
...
function generateReviewAnalysis() {
    ...
    else if (improvementRate >= 40) analysis.recommendations.push('まだ改善の余地があります。');
    else analysis.recommendations.push('基礎からの復習が必要かもしれません。');
...
function displayReviewAnalysis() {
    ...
    if (!container) return;
    const improvementRate = Math.round((analysis.improved / analysis.totalReviewed) * 100);
    const analysisHTML = `<div class="review-analysis"><div class="analysis-header"><span class="analysis-icon">&#128200;</span><span class="analysis-title">復習結果の分析</span></div><div class="analysis-summary"><div class="summary-item improvement"><span class="summary-label">改善率</span><span class="summary-value ${improvementRate >= 70 ? 'good' : improvementRate >= 50 ? 'moderate' : 'needs-work'}">${improvementRate}%</span></div><div class="summary-item"><span class="summary-label">正解</span><span class="summary-value">${analysis.improved}問</span></div><div class="summary-item"><span class="summary-label">不正解</span><span class="summary-value">${analysis.stillWrong}問</span></div></div>${analysis.frequentWrong.length > 0 ? `<div class="frequent-wrong-section"><div class="section-header"><span class="warning-icon">&#9888;</span><span>繰り返し間違える問題</span></div><ul class="frequent-wrong-list">${analysis.frequentWrong.map(q => `<li>問題 ${q.id} - ${q.count}回間違い</li>`).join('')}</ul></div>` : ''}<div class="recommendations-section"><div class="section-header"><span class="bulb-icon">&#128161;</span><span>学習アドバイス</span></div><ul class="recommendations-list">${analysis.recommendations.map(rec => `<li>${rec}</li>`).join('')}</ul></div></div>`;
